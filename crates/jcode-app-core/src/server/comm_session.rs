...
fn cleanup_prepared_visible_spawn_session(session_id: &str) {
    ...
    if let Ok(path) = crate::session::session_path(session_id) {
        drop(std::fs::remove_file(&path));
        drop(std::fs::remove_file(path.with_extension("lock")));
        drop(std::fs::remove_file(path.with_extension("atomic.lock")));
        drop(std::fs::remove_file(path.with_extension("bak")));
    }

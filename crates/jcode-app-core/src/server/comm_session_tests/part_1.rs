...
use crate::server::{SwarmEventType, SwarmMember, SwarmMutationRuntime, VersionedPlan};
...
fn load_session_from_home(home: &std::path::Path, session_id: &str) -> crate::session::Session {
    crate::storage::read_json(&home.join("sessions").join(format!("{session_id}.json")))
...
#[tokio::test]
async fn nonresident_swarm_stop_closes_durably_before_removing_membership() {
    let _guard = crate::storage::lock_test_env();
    let temp_home = tempfile::TempDir::new().expect("temp home");
    crate::env::set_var("JCODE_HOME", temp_home.path());

    let requester = "stop-coordinator";
    let target = "stop-nonresident-target";
    let swarm_id = "stop-durability-swarm";
    let mut persisted = crate::session::Session::create_with_id(target.to_string(), None, None);
    persisted.mark_active();
    persisted.save().expect("persist nonresident target");
    crate::storage::register_active_pid(target, std::process::id());

    let (mut requester_member, _requester_rx) = member(requester, Some(swarm_id), "coordinator");
    requester_member.friendly_name = Some("coordinator".to_string());
    let (mut target_member, _target_rx) = member(target, Some(swarm_id), "agent");
    target_member.report_back_to_session_id = Some(requester.to_string());
    let swarm_members = Arc::new(RwLock::new(HashMap::from([
        (requester.to_string(), requester_member),
        (target.to_string(), target_member),
    ])));
    let swarms_by_id = Arc::new(RwLock::new(HashMap::from([(
        swarm_id.to_string(),
        HashSet::from([requester.to_string(), target.to_string()]),
    )])));
    ...
    assert!(matches!(
        ...
        Ok(ServerEvent::Done { id: 91 })
    ));
    let closed = crate::session::Session::load(target).expect("closed target remains loadable");
    assert_eq!(closed.status, crate::session::SessionStatus::Closed);
    assert_eq!(closed.last_pid, None);
    assert!(!swarm_members.read().await.contains_key(target));
    assert!(
        crate::storage::session_presence()
            .iter()
            .all(|presence| presence.session_id != target)
    );
...
#[tokio::test]
async fn nonresident_swarm_stop_keeps_membership_when_durable_close_cannot_start() {
    let _guard = crate::storage::lock_test_env();
    let temp_home = tempfile::TempDir::new().expect("temp home");
    crate::env::set_var("JCODE_HOME", temp_home.path());

    let requester = "failed-stop-coordinator";
    let target = "missing-nonresident-target";
    let swarm_id = "failed-stop-durability-swarm";
    let (requester_member, _requester_rx) = member(requester, Some(swarm_id), "coordinator");
    let (mut target_member, _target_rx) = member(target, Some(swarm_id), "agent");
    target_member.report_back_to_session_id = Some(requester.to_string());
    let swarm_members = Arc::new(RwLock::new(HashMap::from([
        (requester.to_string(), requester_member),
    ...
    let event = client_event_rx
        .try_recv()
        .expect("durable failure response");
    assert!(matches!(event, ServerEvent::Error { id: 92, .. }));
    assert!(swarm_members.read().await.contains_key(target));
    assert!(
        swarms_by_id
            .read()
...
#[tokio::test]
async fn busy_swarm_stop_restores_live_session_mapping_and_presence() {
    let _guard = crate::storage::lock_test_env();
    let temp_home = tempfile::TempDir::new().expect("temp home");
    crate::env::set_var("JCODE_HOME", temp_home.path());

    let requester = "busy-stop-coordinator";
    let target = "busy-stop-target";
    let swarm_id = "busy-stop-swarm";
    let target_agent = test_agent_with_working_dir(target, "/tmp/busy-stop").await;
    ...
    let event = client_event_rx.try_recv().expect("busy stop response");
    assert!(matches!(event, ServerEvent::Error { id: 93, .. }));
    assert!(
        sessions
            .read()
            .await
            .get(target)
            .is_some_and(|agent| Arc::ptr_eq(agent, &target_agent))
    );
    assert!(swarm_members.read().await.contains_key(target));
    assert!(
        crate::storage::session_presence()
            .iter()
            .any(|presence| presence.session_id == target && presence.pid == std::process::id())
    );

    drop(busy_guard);
    crate::storage::unregister_active_pid(target);
    crate::env::remove_var("JCODE_HOME");
}
...
fn prepare_visible_spawn_session_persists_and_launches_provider_key_for_openrouter_model() {
    ...
    assert!(launched);
    let session = load_session_from_home(temp_home.path(), &session_id);
...
async fn coordinator_identity_falls_back_to_persisted_session_when_agent_busy() {
    ...
    let mut session =
        crate::session::Session::load("coord_busy").expect("load coordinator session");

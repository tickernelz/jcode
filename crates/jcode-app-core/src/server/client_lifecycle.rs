...
type ConnectionDispatchBarriers = std::sync::Mutex<HashMap<String, Arc<Mutex<()>>>>;

fn connection_dispatch_barriers() -> &'static ConnectionDispatchBarriers {
    static BARRIERS: std::sync::OnceLock<ConnectionDispatchBarriers> = std::sync::OnceLock::new();
    BARRIERS.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}

pub(super) fn connection_dispatch_barrier(connection_id: &str) -> Arc<Mutex<()>> {
    let mut barriers = connection_dispatch_barriers()
        .lock()
...
pub(super) async fn acquire_connection_dispatch_lease(
    client_connections: &Arc<RwLock<HashMap<String, ClientConnectionInfo>>>,
    connection_id: &str,
    session_id: &str,
) -> Option<tokio::sync::OwnedMutexGuard<()>> {
    let barrier = connection_dispatch_barrier(connection_id);
    let lease = barrier.lock_owned().await;
    client_connections
        .read()
        .await
        .get(connection_id)
...
pub(super) fn remove_connection_dispatch_barrier(connection_id: &str) {
    connection_dispatch_barriers()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner)
...
pub(super) async fn handle_client(
    ...
    let initial_request = loop {
        ...
        let read_result = tokio::select! {
            biased;
            _ = runtime_cancellation.cancelled() => return Ok(()),
    ...
    loop {
        let request = if let Some(request) = pending_request.take() {
            ...
            _ = runtime_cancellation.cancelled() => {
                crate::logging::info(&format!(
                    "Server runtime cancelled connection {}; entering owned cleanup",
            ...
            bus_event = bus_rx.recv(), if client_subscribed => {
                match bus_event {
                    ...
                    Ok(BusEvent::CompactionModelChanged { model }) => {
                        drop(client_event_tx.send(ServerEvent::CompactionModelChanged {
                            id: 0,
                            model,
                            error: None,
                        }));
        ...
        // Removal from `client_connections` is the takeover revocation
        // transaction. Revalidate ownership after decode and immediately before
        // any request can acknowledge, cancel, mutate a session, or start a turn.
        // This closes the race where readable socket input won the select after
        // a successor had removed and signalled the old owner.
        let Some(_dispatch_lease) = acquire_connection_dispatch_lease(
            &client_connections,
            &client_connection_id,
            &client_session_id,
        )
        .await
            ...
            crate::logging::warn(&format!(
                "Rejecting request from revoked connection {} for session {}",
                client_connection_id, client_session_id
            ));
            break;
        };
        ...
        match request {
            ...
            Request::Clear { id } => {
                ...
                let clear_result = handle_clear_session(
                    id,
                    client_selfdev,
                ...
                .await;
                if let Err(message) = clear_result {
            ...
            Request::SwitchAnthropicAccount { id, label } => {
                handle_switch_anthropic_account(id, label, &sessions, &agent, &client_event_tx)
                    .await;
            }

            Request::SwitchOpenAiAccount { id, label } => {
                handle_switch_openai_account(id, label, &sessions, &agent, &client_event_tx).await;
            }
    ...
    .await?;
    remove_connection_dispatch_barrier(&client_connection_id);
    Ok(())
}

mod processing;
pub(super) use processing::process_message_streaming_mpsc;
use processing::{
    cancel_processing_message, clear_soft_interrupts, move_tool_to_background,
    names_only_available_models_event, queue_soft_interrupt, start_processing_message,
    try_available_models_snapshot,
};

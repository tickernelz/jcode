...
impl Provider for NeverEndingStreamProvider {
    ...
    fn model(&self) -> String {
        "never-ending-stream".to_string()
    }

    fn set_model(&self, model: &str) -> Result<()> {
        if model == "never-ending-stream" || model.ends_with(":never-ending-stream") {
            Ok(())
        } else {
            anyhow::bail!("unsupported never-ending-stream model {model}")
        }
    }
...
impl Provider for CompleteImmediatelyProvider {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        if model == "complete-immediately" || model.ends_with(":complete-immediately") {
            Ok(())
        } else {
            anyhow::bail!("unsupported complete-immediately model {model}")
        }
    }
...
impl Provider for FanoutStreamProvider {
    ...
    fn model(&self) -> String {
        "fanout-stream".to_string()
    }

    fn set_model(&self, model: &str) -> Result<()> {
        if model == "fanout-stream" || model.ends_with(":fanout-stream") {
            Ok(())
        } else {
            anyhow::bail!("unsupported fanout-stream model {model}")
        }
    }

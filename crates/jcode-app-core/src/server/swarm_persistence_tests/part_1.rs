...
#[test]
fn closed_session_tombstone_prunes_interrupted_nonterminal_swarm_stop() {
    let dir = tempfile::TempDir::new().expect("tempdir");
    let _env = test_env(&dir);
    let session_id = "session-interrupted-swarm-stop";
    let mut session = crate::session::Session::create_with_id(
        session_id.to_string(),
        None,
        Some("interrupted stop".to_string()),
    );
    session.save().expect("save active session");
    ...
    let member = SwarmMember {
        ...
        event_tx,
        event_txs: HashMap::new(),
        working_dir: Some(PathBuf::from("/tmp/swarm-stop-tombstone")),
        swarm_id: Some("swarm-stop-tombstone".to_string()),
        swarm_enabled: true,
        status: "ready".to_string(),
        detail: None,
    ...
    persist_swarm_state("swarm-stop-tombstone", None, None, &[member]);

    // Simulate the crash point after durable close but before membership and
    // queue cleanup/persistence.
    session.status = crate::session::SessionStatus::Closed;
    session.save().expect("persist close tombstone");

    let loaded = load_runtime_state();
    assert!(!loaded.members.contains_key(session_id));
    assert!(
        loaded
            .swarms_by_id
            .get("swarm-stop-tombstone")
            .is_none_or(HashSet::is_empty)
    );
    assert!(!state_path("swarm-stop-tombstone").exists());
}

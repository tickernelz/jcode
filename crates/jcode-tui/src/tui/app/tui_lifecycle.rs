#[path = "tui_lifecycle_reload.rs"]
mod tui_lifecycle_reload;
use super::*;
...
impl App {
    ...
    pub(super) fn new_minimal_with_session(
        ...
        let mut app = Self {
            ...
            pending_remote_compaction_model: None,
            ...
            pending_remote_account_switch: None,
    ...
    pub fn new(provider: Arc<dyn Provider>, registry: Registry) -> Self {
        ...
        let startup_account_reconciliation_error =
            crate::server::reconcile_pending_account_transition_for_admission(&provider)
                .err()
        ...
        if let Some(error) = startup_account_reconciliation_error {
            crate::logging::error(&format!(
                "Local TUI startup account reconciliation remains blocked: {error}"
            ));
            app.push_display_message(DisplayMessage::error(format!(
                "Provider account reconciliation must complete before the next model request: {error}"
            )));
            app.status_notice = Some((
                "Provider account reconciliation blocked".into(),
                Instant::now(),
            ));
        }

...
#[test]
fn test_recover_session_without_tools_uses_only_active_rewind_branch() {
    let mut app = create_test_app();
    app.session.add_message(
        Role::User,
        vec![ContentBlock::Text {
            text: "active before recovery".to_string(),
            cache_control: None,
        }],
    );
    app.session.add_message(
        Role::Assistant,
        vec![ContentBlock::Text {
            text: "archived before recovery".to_string(),
            cache_control: None,
        }],
    );
    let expected_archived_count = app.session.messages.len().saturating_sub(1);
    app.session
        .rewind_active_branch_through(0)
        .expect("rewind source session");
    app.session.save().expect("save source session");
    let old_session_id = app.session.id.clone();
    ...
    assert!(app.session.messages.iter().all(|message| {
        !message
            .content_preview()
            .contains("archived before recovery")
    }));
    let old_session = Session::load(&old_session_id).expect("reload canonical source");
    assert!(old_session.messages.iter().any(|message| {
        message.content_preview() == "archived before recovery"
    }));
    assert_eq!(
        old_session.archived_message_ids.len(),
        expected_archived_count,
        "rewind must archive every canonical message after the retained branch point"
    );

    let _ = std::fs::remove_file(crate::session::session_path(&old_session_id).unwrap());
    let _ = std::fs::remove_file(crate::session::session_path(&app.session.id).unwrap());
}

...
#[test]
fn test_handle_server_event_history_applies_exact_runtime_identity_to_session_mirror() {
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
    let identity = jcode_provider_core::ExactRuntimeIdentity {
        ...
        route: jcode_provider_core::RouteSelection {
            ...
            runtime_key: jcode_provider_core::RuntimeKey::OpenAiCompatible {
                profile_id: Some("sub2api-codex".to_string()),
            },
            api_method: "openai-responses".to_string(),
            provider_label: "Sub2API".to_string(),
            detail: "profile:sub2api-codex".to_string(),
        },
        account_label: Some("primary".to_string()),
        account_id: Some("stable-account-id".to_string()),
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::History {
            id: 2,
            session_id: "session_exact_identity".to_string(),
            messages: vec![],
            images: vec![],
            provider_name: Some("Sub2API".to_string()),
            provider_model: Some("gpt-5.5".to_string()),
            exact_runtime_identity: Some(identity.clone()),
            subagent_model: None,
            autoreview_enabled: None,
            autojudge_enabled: None,
            available_models: vec![],
    ...
    assert_eq!(app.session.exact_runtime_identity, Some(identity));
}

...
impl App {
    ...
    pub(super) fn finalize_model_switch(&mut self, model_request: &str) -> anyhow::Result<String> {
        self.finalize_applied_model_switch(model_request, None, None, None)
    }

    pub(super) fn finalize_route_selection(
        &mut self,
        model_request: &str,
        selection: &crate::provider::RouteSelection,
    ) -> anyhow::Result<String> {
        let active_model = self.provider.model();
        self.finalize_applied_model_switch(
            model_request,
            Some(crate::provider::persisted_session_model_for_route(
                selection,
                &active_model,
            )),
            Some(selection.api_method.clone()),
            Some(selection.runtime_key.stable_id()),
        )
    }

    pub(super) fn finalize_applied_model_switch(
        &mut self,
        model_request: &str,
        ...
        let active_model = self.provider.model();
        let mut candidate = previous_session.clone();
        candidate.provider_session_id = None;
        candidate.provider_session_identity = None;
        candidate.provider_key = provider_key.or_else(|| {
        ...
        candidate.route_api_method = route_api_method.or_else(|| {
            crate::provider::MultiProvider::route_api_method_after_model_switch(
                model_request,
                previous_session.route_api_method.as_deref(),
            )
        });
        candidate.model = Some(persisted_model.unwrap_or_else(|| active_model.clone()));
        let effective_identity = self.provider.exact_runtime_identity();
        let identity_changed = candidate.exact_runtime_identity != effective_identity
            || candidate.model != previous_session.model
            || candidate.provider_key != previous_session.provider_key
            || candidate.route_api_method != previous_session.route_api_method;
        if identity_changed {
            candidate.exact_runtime_identity = effective_identity;
        ...
        if let Err(error) = candidate.save() {
            if let Err(rollback) = crate::provider::set_model_with_auth_refresh(
                ...
                self.set_status_notice(
                    "Unsafe provider identity after model-switch rollback failure; shutting down",
                );
                anyhow::bail!(
                    "{error}; failed to roll back exact provider route '{previous_route_request}': {rollback}"
                );
            }
            return Err(error);
        }
        ...
        if identity_changed && let Err(error) = self.reset_provider_view_after_identity_transition()
        {
            self.should_quit = true;
            self.set_status_notice(
                "Unsafe in-memory context after provider identity transition; shutting down",
            );
            return Err(error);
        }
        Ok(active_model)
    ...
    pub(super) fn set_reasoning_effort_transactional(
        &mut self,
        effort: &str,
        ...
        let previous_effort = candidate.reasoning_effort.clone();
        candidate.reasoning_effort = current.clone();
        let effective_identity = self.provider.exact_runtime_identity();
        let identity_changed = candidate.exact_runtime_identity != effective_identity
            || candidate.reasoning_effort != previous_effort;
        if identity_changed {
            candidate.provider_session_id = None;
            candidate.provider_session_identity = None;
            candidate.exact_runtime_identity = effective_identity;
        ...
        if let Err(error) = candidate.save() {
                ...
                .set_reasoning_effort(previous.as_deref().unwrap_or(""))
            {
                self.should_quit = true;
                self.set_status_notice(
                    "Unsafe reasoning identity after rollback failure; shutting down",
                );
                anyhow::bail!("{error}; failed to roll back reasoning effort: {rollback}");
            }
            return Err(error);
        }
        self.session = candidate;
        if identity_changed {
            ...
            if let Err(error) = self.reset_provider_view_after_identity_transition() {
                self.should_quit = true;
                self.set_status_notice(
                    "Unsafe in-memory context after reasoning identity transition; shutting down",
                );
                return Err(error);
            }
        }
    ...
    fn apply_provider_switch_for_failover(
        ...
        let active_model = self.provider.model();
        self.finalize_model_switch(&active_model)
    }
    ...
    pub(super) fn cycle_model(&mut self, direction: i8) {
        ...
        match self
            ...
            .and_then(|()| self.finalize_model_switch(&next_model).map(|_| ()))
        {
            Ok(()) => {
    ...
    pub(super) fn cycle_effort(&mut self, direction: i8) {
        ...
        match self.set_reasoning_effort_transactional(next_effort) {
            Ok(_) => {
                let label = effort_display_label(next_effort);
    ...
    pub(super) fn handle_turn_error(&mut self, error: impl Into<String>) {
        ...
        if is_request_payload_too_large_error(&error) {
            ...
            // Suppress oversized images in the provider projection so a manual
            // resubmit (or auto-poke retry) goes through while raw history stays canonical.
            let stripped = self.session.suppress_oversized_images_for_provider(
                crate::compaction::PAYLOAD_IMAGE_CHAR_BUDGET,
            );
            if stripped > 0 {
                if let Err(save_error) = self.session.save() {
                    self.push_display_message(DisplayMessage::error(format!(
                        "Error: {error} Provider image recovery could not be persisted: {save_error:#}"
                    )));
                ...
                self.push_display_message(DisplayMessage::error(format!(
                    "Error: {} Suppressed {} oversized image(s) for provider retry.",
                    error, stripped
                )));
            } else {
                self.push_display_message(DisplayMessage::error(format!(
    ...
    pub(super) fn auto_recover_context_limit(&mut self) -> Option<String> {
        ...
        if manager.engine() == crate::config::CompactionEngine::Lcm {
            ...
            let action =
                manager.ensure_lcm_context_fits(&mut self.session, Arc::clone(&self.provider));
            ...
            return match action {
                crate::compaction::CompactionAction::HardCompacted(dropped) => {
                    ...
                    Some(format!(
                        "⚡ LCM recovery chain summarized {dropped} old messages. You can continue."
                    ))
                }
                _ => {
                    crate::logging::error("[auto_recover] LCM fallback chain did not compact");
                    None
                }
            };
        }
    ...
    /// suppressing oversized inline images (oldest-first) in the provider
    /// projection, then retrying the turn. Returns true if the retry succeeded.
    ///
    ...
    pub(super) async fn try_recover_payload_too_large_and_retry(
        ...
        if let Err(error) = self.session.save() {
            crate::logging::error(&format!(
                "Request-too-large provider projection could not be persisted before retry: {error:#}"
            ));
            return false;
        }

        // Provider projection changed: drop the local materialized scratch copy so the
        ...
        self.push_display_message(DisplayMessage::system(format!(
            "⚡ Request was too large; suppressed {} oversized image(s) for provider retry...",
            stripped
        )));
                    ...
                    match manager.materialize_lcm_context(&mut self.session) {
                        ...
                        Err(error) => {
                            crate::logging::warn(&format!(
                                "Failed to materialize durable LCM context: {error}"
                            ));
                            self.push_display_message(DisplayMessage::error(format!(
...
mod model_command;
pub(super) use model_command::handle_model_command;

    ...
    pub(super) fn submit_input(&mut self) {
        ...
        if let Err(error) = self.reconcile_local_account_transition_for_admission() {
            ...
            self.push_display_message(DisplayMessage::error(format!(
                "Cannot start a model turn while account identity is reconciling: {error}"
            )));
        ...
        // A new user turn creates a branch from the rewound frontier. Restoring
        // the old snapshot after this point would delete newly committed work.

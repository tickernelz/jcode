...
impl BackgroundTaskManager {
    ...
    pub async fn register_detached_task(
        ...
        // This record is the only restart-recoverable owner of an OS-detached
        // process. Do not publish success or let the caller disarm its process
        // guard until the status is atomically durable.
        crate::storage::write_json(&info.status_file, &status)?;
    ...
    pub async fn adopt_with_options(
        ...
        // The status record is the durable ownership transfer. If it cannot be
        // published, abort and join the task while we still own its handle.
        // Dropping a JoinHandle here would detach the nested tool task.
        if let Err(error) = crate::storage::write_json(&status_path, &initial_status) {
            handle.abort();
            let _ = handle.await;
            return Err(error);
        }
